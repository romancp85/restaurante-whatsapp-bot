...
const CORTESIA = ['GRACIAS', 'OK', 'DALE', 'ESTA BIEN', 'ENTENDIDO', 'PERFECTO', 'LISTO', '👍', 'BYE', 'ADIOS', 'EXCELENTE', 'MUCHAS GRACIAS', 'YA QUEDO'];
const FEEDBACK_NEGATIVO = ['FALTO', 'FALTA', 'MALO', 'FRIO', 'SALSA', 'TARDE', 'EQUIVOCADO', 'ERROR', 'NO LLEGO', 'POCA', 'POCO'];

export const handleInitialFlow = async (userId, businessId, text, cart, auth, lastOrder = null) => {
    ...
    if (esCortesia || esEmoji) {
        ...
        if (lastOrder) {
            respuesta = (lastOrder.estado === 'Pendiente de Pago') 
    ...
    if (lastOrder && text.length < 40) {
        ...
        const keywordsStatus = ['donde', 'viene', 'estatus', 'pedido', 'orden', 'va', 'falta', 'confirmar', 'llega', 'tarda'];
        if (keywordsStatus.some(k => lowerText.includes(k))) {
            let statusMsg = `Tu pedido *#${lastOrder.numero_pedido}* está: *${lastOrder.estado}*.\n\n`;
            statusMsg += (lastOrder.estado === 'Pendiente de Pago') 
        ...
        if (currentState === 'ESPERANDO_COMPROBANTE' && text.length < 20 && !lowerText.includes('menu')) {
    ...
    if (cart.items.length === 0 && esEstadoInicialValido && (esMensajeCorto || esSaludo)) {
        console.log(`[flowService] 👋 Manejando inicio legal (Estado: ${currentState}).`);

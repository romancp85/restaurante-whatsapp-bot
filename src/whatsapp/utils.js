  ...
  try {
    ...
    if (menuItems.length === 0) {
      return await sendMessage(
        to,
        "Lo sentimos, hoy no tenemos productos disponibles. 😴",
        auth,
      );
    }

    // =====================================================
    // CONSTRUCCIÓN VISUAL DEL MENÚ
    // =====================================================
    ...
    menuText += "_Ejemplo: 'Quiero 2 pizzas hawaianas y una coca'_";
    ...
    if (cart?.items?.length > 0) {
      const interactivePayload = {
        ...
        interactive: {
          ...
          body: {
            text: "Tienes productos en tu carrito. ¿Qué deseas hacer? 👇",
          },
...
export const sendCartSummary = async (to, cart, businessId, auth) => {
  try {
    if (!cart.items || cart.items.length === 0) {
      return await sendMessage(
        to,
        "🛒 Tu carrito está vacío. ¡Echa un vistazo al menú! 🍔",
        auth,
      );
    }
    ...
    cart.items.forEach((item, index) => {
      ...
      if (item.opcionesSeleccionadas?.length > 0) {
        item.opcionesSeleccionadas.forEach((opt) => {
          const precioExtra =
            opt.precioExtra > 0 ? ` (+${formatPrice(opt.precioExtra)})` : "";
    ...
    if (envio > 0 && !esPickup)
      summaryText += `*Envío:* ${formatPrice(envio)}\n`;
    ...
    await sendMessage(
      to,
      "Hubo un problema al generar el resumen, pero tu carrito está a salvo. Escribe *CARRITO* para reintentar.",
      auth,
    );
  }
};
...
export const sendPaymentMethodOptions = async (to, businessId, auth) => {
  try {
    ...
    if (!restaurante) {
      logger.error(
        `[SaaS Error] No se encontró el restaurante ${businessId} para cargar pagos.`,
      );
      return await sendMessage(
        to,
        "Lo sentimos, hay un error en la configuración de pagos. Por favor, intenta más tarde.",
        auth,
      );
    }
    ...
    await sendMessage(
      to,
      "Por ahora solo aceptamos Efectivo. ¿Te parece bien?",
      auth,
    );
  }
};

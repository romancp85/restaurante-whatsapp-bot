...
if (missingEnv.length > 0) {
  logger.error(
    "[server] ❌ FATAL: Variables de entorno requeridas no definidas:",
    missingEnv,
  );
  process.exit(1);
}
...
const SocketServer =
  socketPkg.Server || socketPkg.default?.Server || socketPkg.default;
const app = express();
...
io.on("connection", (socket) => {
  ...
  logger.info(
    `[server Socket] Dashboard conectado a sala: ${socket.businessId}`,
  );
});
...
mongoose
  ...
  .then(async () => {
    logger.info("[server]✅ Conexión a MongoDB exitosa");
    try {
      // Importación dinámica: el worker ahora encontrará global.io listo
      await import("./workers/orderWorker.js");
      logger.info("[server]🤖 Workers de BullMQ activos");
    } catch (e) {
      logger.error("[server]💥 ERROR AL CARGAR EL WORKER:", e);
    }
    httpServer.listen(PORT, () =>
      logger.info(`[server] 🚀 Servidor en puerto ${PORT}`),
    );
  })
  .catch((err) => {
    logger.error("[server]💥 Error crítico MongoDB:", err.message);
    process.exit(1);
  });

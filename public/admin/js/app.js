...
console.log("🚀 Módulo App iniciado...");
...
const init = async () => {
    try {
        ...
        socket.on('connect', () => {
            ...
            if (badge) {
                badge.innerHTML = '<span class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span> SYSTEM LIVE';
                badge.className = 'flex items-center gap-2 bg-green-50 text-green-600 px-3 py-1 rounded-full text-[10px] font-black border border-green-200';
            }
        });

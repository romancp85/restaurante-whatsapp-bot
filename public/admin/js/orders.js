...
export const orders = {
    ...
    async load() {
        ...
        } catch (e) { console.error("❌ Error load orders:", e); }
    },
    ...
    closeModal() {
        ...
        ['cust-name', 'cust-phone', 'cust-address', 'prod-select', 'item-notes'].forEach(id => {
    ...
    renderTempOrder() {
        ...
        if (this.tempItems.length === 0) {
            list.innerHTML = '<p class="text-center text-slate-400 text-xs py-10 italic">Carrito vacío</p>';
        ...
        list.innerHTML = this.tempItems.map((item, index) => {
        ...
        if(subtotalDisp) subtotalDisp.innerText = `$${(subtotal / 100).toFixed(2)}`;
        if(deliveryDisp) deliveryDisp.innerText = `+$${(this.deliveryFee / 100).toFixed(2)}`;
    ...
    async submitFinalOrder() {
        ...
        const payload = {
            customerName: document.getElementById('cust-name').value || "Cliente Mostrador",
            ...
            direccionEntrega: this.currentDeliveryMode === 'DELIVERY' ? document.getElementById('cust-address').value : 'RECOGIDA EN TIENDA',
    ...
    render(data) {
        ...
        if (!data || data.length === 0) {
            grid.innerHTML = '<div class="col-span-full text-center py-20 text-slate-300 font-bold uppercase">Sin pedidos activos</div>';
            return;
        }
                    ...
                    <div class="space-y-2 mb-6">
                        ${p.items.map(i => `
                            <div class="text-sm font-bold text-slate-700">
                                ...
                                ${i.notas ? `<p class="text-[10px] text-orange-400 italic">"${i.notas}"</p>` : ''}
                            </div>`).join('')}
                    </div>
    ...
    getActionButtons(p) {
        const btnBase = "w-full text-[10px] font-black py-3 rounded-2xl transition-all uppercase tracking-widest";
        if (p.estado === 'Pendiente de Pago') return `<button onclick="window.changeStatus('${p._id}', 'Confirmado')" class="${btnBase} bg-amber-500 text-white hover:bg-amber-600">VALIDAR PAGO</button>`;
        if (p.estado === 'Pendiente') return `<button onclick="window.changeStatus('${p._id}', 'Confirmado')" class="${btnBase} bg-indigo-600 text-white">CONFIRMAR</button>`;
        if (p.estado === 'Confirmado' || p.estado === 'En Preparación') {
            // 🌟 LÓGICA TOP-TIER: El nombre del botón depende del modo de entrega
            const label = p.entregaMode === 'PICKUP' ? '✅ MARCAR COMO LISTO' : '🛵 ENVIAR REPARTIDOR';
        ...
        if (p.estado === 'En Camino') return `<button onclick="window.changeStatus('${p._id}', 'Entregado')" class="${btnBase} bg-slate-800 text-white">FINALIZAR</button>`;
            return '';
        },

    updateStats(pedidos) {
        const pending = pedidos.filter(p => ['Pendiente', 'Pendiente de Pago'].includes(p.estado)).length;
        const kitchen = pedidos.filter(p => ['Confirmado', 'En Preparación'].includes(p.estado)).length;
        ...
        if (document.getElementById('stat-kitchen')) document.getElementById('stat-kitchen').innerText = kitchen;
        if (document.getElementById('stat-delivery')) document.getElementById('stat-delivery').innerText = delivery;
        if (document.getElementById('stat-sales')) document.getElementById('stat-sales').innerText = `$${total.toLocaleString('es-MX', { minimumFractionDigits: 2 })}`;
    }
};
...
window.removeTempItem = (idx) => { orders.tempItems.splice(idx, 1); orders.renderTempOrder(); };
